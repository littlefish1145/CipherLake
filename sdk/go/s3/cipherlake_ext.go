...
// This is a CipherLake-specific extension that uses the /_vector_search endpoint.
func (c *Client) VectorSearch(ctx context.Context, query string, topK int) ([]SearchResult, error) {
	...
	resp, err := c.doCipherLakeRequest(ctx, http.MethodPost, "/_vector_search", body)
	if err != nil {
...
// This is a CipherLake-specific extension that uses the /{bucket}/_fts endpoint.
func (c *Client) FTSSearch(ctx context.Context, bucket, query string, topK int) ([]FTSResult, error) {
	...
	resp, err := c.doCipherLakeRequest(ctx, http.MethodPost, path, body)
	if err != nil {
...
// This is a CipherLake-specific extension that uses the resumable upload API.
func (c *Client) ResumableUpload(ctx context.Context, bucket, key string, reader io.Reader) error {
	...
	resp, err := c.doCipherLakeRequest(ctx, http.MethodPost, initPath, initJSON)
	if err != nil {
	...
	completeResp, err := c.doCipherLakeRequest(ctx, http.MethodPut, completePath, completeJSON)
	if err != nil {
...
// doCipherLakeRequest makes an HTTP request to a CipherLake-specific API endpoint.
func (c *Client) doCipherLakeRequest(ctx context.Context, method, path string, body []byte) (*http.Response, error) {
