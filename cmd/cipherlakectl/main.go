...
var rootCmd = &cobra.Command{
	Use:   "cipherlakectl",
	Short: "CipherLake CLI - Management tool for CipherLake storage system",
	Long: `CipherLake Control (cipherlakectl) is a command-line tool for managing
CipherLake S3-compatible storage system.`,
...
func init() {
	rootCmd.PersistentFlags().StringVar(&address, "address", "http://localhost:8080", "CipherLake server address")
...
func serverRequest(method, path string) (*http.Response, error) {
	...
	user := os.Getenv("CIPHERLAKE_ADMIN_USER")
	pass := os.Getenv("CIPHERLAKE_ADMIN_PASSWORD")
	if user == "" && ak != "" {
		user = ak
		pass = sk
	}
