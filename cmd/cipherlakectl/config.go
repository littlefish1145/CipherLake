...
// CLIConfig represents the cipherlakectl configuration file.
...
func configPath() string {
	...
	return filepath.Join(home, ".cipherlakectl.yaml")
}
...
func deriveEncryptionKey() ([]byte, error) {
	...
	if err != nil {
		machineID = "cipherlakectl-default-key"
	}

	h := hkdf.New(sha256.New, []byte(machineID), []byte("cipherlakectl-config-encryption"), nil)
	key := make([]byte, 32)
...
var configCmd = &cobra.Command{
	Use:   "config",
	Short: "Manage cipherlakectl configuration",
	Long:  "Manage the ~/.cipherlakectl.yaml configuration file",
}
...
var configValidateCmd = &cobra.Command{
	Use:   "validate",
	Short: "Validate a CipherLake server configuration file",
	Long:  "Validate a CipherLake server configuration file against the schema and report errors and warnings",
	RunE: func(cmd *cobra.Command, args []string) error {
...
func init() {
	...
	configValidateCmd.Flags().StringP("file", "f", "", "Path to the CipherLake server config file to validate")
